  ...
  <div class="">
    <div
      ...
      <div class="flex justify-center text-white w-full h-full flex-col">
        <h1 class="text-2xl">
          <strong class="text-primary">Б/у автозапчасти </strong>онлайн от 13 европейских продавцов
        </h1>
        <p class="mt-2">Выбирайте из 70 000 б/у запчастей по доступным ценам в одном магазине</p>
      </div>
      <div class="w-full justify-center flex h-full items-center">
        <div class="flex w-full gap-2 flex-col bg-white p-4 rounded-lg">
          ...
          <n-select v-model:value="selectedManufacturer"
                    filterable
                    clearable
                    placeholder="Марка"
...
function search() {
  router.push({
    ...
    params: {
      manufacturerId: selectedManufacturer.value != undefined ? selectedManufacturer?.value : null
    }
  });
}
...
function getCarModels(value: number) {
  axiosInstance.get<{
    results: IDefaultAPI[]
  }>(`/api/car/models/?manufacturer=${value}`).then(res => {
